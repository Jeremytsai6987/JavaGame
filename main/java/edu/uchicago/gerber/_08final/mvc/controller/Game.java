...
public class Game implements Runnable, KeyListener {
    ...
    public static final Dimension DIM = new Dimension(1400, 1000); //the dimension of the game.
    ...
    public void keyPressed(KeyEvent e) {
        ...
        switch (keyCode) {
            ...
            case LEFT:
                ...
                falcon.setLastDirection(180);
                break;
            case RIGHT:
                falcon.setTurnState(Falcon.TurnState.RIGHT);
                falcon.setLastDirection(0);
                break;
            case DOWN:
    ...
    public void keyReleased(KeyEvent e) {
        ...
        switch (keyCode) {
            case FIRE:
                if (falcon.getLastWeapon() == Falcon.WEAPON_STATE.BULLET){

                    int falconOrientation = CommandCenter.getInstance().getFalcon().getOrientation();
                    Bullet bullet = new Bullet(falcon, falconOrientation);
                    CommandCenter.getInstance().getOpsQueue().enqueue(bullet, GameOp.Action.ADD);
                    break;
                }else{
                    int falconOrientation = CommandCenter.getInstance().getFalcon().getOrientation();
                    Spear spear = new Spear(falcon, falconOrientation);
            ...
            case RIGHT:
            case UP:
                if (falcon.getTurnState() != Falcon.TurnState.IDLE) {
            ...
            case SWITCH:
                if(falcon.getLastWeapon() == Falcon.WEAPON_STATE.BULLET){
                    falcon.setWeapon(Falcon.WEAPON_STATE.SPEAR);

                }else{
                    falcon.setWeapon(Falcon.WEAPON_STATE.BULLET);
                }
                break;
