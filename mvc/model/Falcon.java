...
public class Falcon extends Sprite {
	...
	public static final int MOVEMENT_SPEED = 10; // You can adjust this value as needed
	...
	public Falcon() {
		...
		//rasterMap.put(ImageState.FALCON, loadGraphic("/imgs/Duck/Gifs/inhaling.gif") ); //normal ship
		//rasterMap.put(ImageState.FALCON_THR, loadGraphic("/imgs/Duck/Gifs/inhaling.gif") ); //normal ship thrusting
		//rasterMap.put(ImageState.FALCON_PRO, loadGraphic("/imgs/Duck/Gifs/inhaling.gif") ); //protected ship (green)
		//rasterMap.put(ImageState.FALCON_PRO_THR, loadGraphic("/imgs/Duck/Gifs/inhaling.gif") ); //green thrusting
		rasterMap.put(ImageState.FALCON_LEFT, loadGraphic("/imgs/Duck/Gifs/f_smash.gif"));
		rasterMap.put(ImageState.FALCON_RIGHT, loadGraphic("/imgs/Duck/Gifs/inhaling.gif"));
	...
	public void move() {
		...
		final int MAX_VELOCITY = 20; //from 39 to 20


		//apply some thrust vectors using trig.
		if (thrusting) {
			double vectorX = Math.cos(Math.toRadians(getLastDirection()))
					* THRUST;
			double vectorY = Math.sin(Math.toRadians(getLastDirection()))
					* THRUST;
		...
		switch (turnState) {
			case LEFT:
				...
				currentImageState = ImageState.FALCON_LEFT;
				break;
			case RIGHT:
				setCenter(new Point(getCenter().x + MOVEMENT_SPEED, getCenter().y));
				setOrientation(ORIENTATION_RIGHT);
				currentImageState = ImageState.FALCON_RIGHT;
				break;
			case DOWN:
